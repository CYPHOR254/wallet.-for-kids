...
router.post("/register_child", async (req, res) => {
  const { parent_email, username, email, Idnumber, pin, phoneNo, DOB } =
    req.body;

  // Save parent details to
  try {
    db.query(
      "INSERT INTO ChildrenDB (parentid, username, email, Idnumber, pin, phoneNo, DOB) VALUES (?, ?,?, ?, ?, ?, ?)",
      [parent_email, username, email, Idnumber, pin, phoneNo, DOB]
    );
  } catch (err) {
    console.error(err);
  ...
  try {
    ...
    db.query(
      "INSERT INTO USERSDB ( username ,email ,pin , phoneNO ,Idnumber,password, DOB ,parent_email ,isActive ,isBlocked ,trials , firstLogin) VALUES (?,?,?, ?, ?, ?, ?, ?, ?,? ,? ,?)",
      [
        username,
        email,
        pin,
    ...
    try {
      ...
      db.query(
        "INSERT INTO accountDB ( childId, accountNo, accountName, available_balance, current_balance, status, createdAt, createdBy, lastTransactionAmount, lastTransactionType) VALUES (?, ?,?, ?, ?, ?, ?, ?, ?, ?)",
        [
          email,
          accountNo,
          username,
    ...
    res
      .status(200)
      .json({ success: "User and account registered successfully", uuid });
