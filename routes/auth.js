...
router.post("/register", async (req, res) => {
  ...
  try {
    ...
    db.query(
      "INSERT INTO USERSDB (username, phoneNo, Idnumber,DOB, password , email,   pin, isActive ,isBlocked ,trials , firstLogin) VALUES (?, ?, ?, ?, ?, ?, ?,? ,? ,? ,?)",
      [
        username,
        phoneNo,
        Idnumber,
...
router.put("/change-password", async (req, res) => {
  ...
  try {
    ...
    if (newEmail) {
      ...
      updateQuery =
        "UPDATE USERSDB SET password = ?, email = ? WHERE email = ?";
    }
